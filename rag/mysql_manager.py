...
try:
    ...
    print("⚠️  MySQL相关库未安装，使用模拟MySQL客户端")
...
class MySQLManager:
    ...
    def _create_tables(self):
        """创建数据库表"""
        try:
            if hasattr(self, 'engine') and MYSQL_AVAILABLE:
    ...
    def get_session_conversations(
        ...
        try:
            ...
            with self.get_session() as session:
                conversations = session.query(ConversationRecord)\
    ...
    def get_user_sessions(
        ...
        try:
            if hasattr(self, 'mock_manager'):
                # 模拟实现
                return [s for s in self.mock_manager.sessions.values() 
                       if s.get('user_id') == user_id][:limit]
            
            with self.get_session() as session:
                sessions = session.query(SessionRecord)\
    ...
    def get_conversation_stats(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.get_session() as session:
                ...
                period_conversations = session.query(ConversationRecord)\
                    .filter(ConversationRecord.created_at >= start_date).count()
                
                # 统计会话数量
                total_sessions = session.query(SessionRecord).count()
                period_sessions = session.query(SessionRecord)\
                    .filter(SessionRecord.created_at >= start_date).count()
