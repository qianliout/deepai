    ...
    def _check_dependencies(self):
        ...
        if missing_packages:
            ...
            self._add_result(
                "依赖库检查",
                "success",
                f"所有 {len(required_packages)} 个依赖库已安装",
                {"available": available_packages}
            )
        ...
        try:
            ...
            if issues:
                self._add_result(
                    ...
                    {"issues": issues, "config_keys": list(config_dict.keys())}
                )
            else:
                self._add_result(
                    "配置检查",
    ...
    def _check_directories(self):
        ...
        if issues:
            self._add_result(
                "目录权限检查",
                "error",
                f"发现 {len(issues)} 个目录问题",
                {"issues": issues, "created": created_dirs}
            )
        else:
            self._add_result(
                "目录权限检查",
                "success",
                f"所有 {len(directories)} 个目录正常",
            ...
            try:
                ...
                if response.status_code == 200:
                    self._add_result(
                        ...
                        "success",
                        "API连接正常",
                        {"model": config.llm.model_name, "status_code": response.status_code}
                    )
                else:
                    self._add_result(
                        "通义百炼API检查",
    ...
    def _check_database_connections(self):
        ...
        try:
            ...
            self._add_result(
                ...
                "warning",
                f"Redis连接失败: {e}",
                {"error": str(e), "host": config.redis.host, "port": config.redis.port}
            )
    ...
    def _check_model_availability(self):
        ...
        try:
            ...
            self._add_result(
                ...
                "error",
                f"嵌入模型加载失败: {e}",
                {"error": str(e), "model_name": config.embedding.model_name}
            )
